...
import com.vs.mvp.net.converter.GsonConverterFactory;
...
public class XApi {
    ...
    private OkHttpClient getClient(String baseUrl, NetProvider provider) {
        ...
        if (handler != null) {
            builder.addInterceptor(new XInterceptor(handler));//请求参数拦截-打印输出
        }
        ...
        if (provider.configLogEnable()) {
            LogInterceptor logInterceptor = new LogInterceptor();//返回日志拦截-打印输出
     ...
     * @return
     */
    public static <T extends BaseResults> FlowableTransformer<T, T> getScheduler() {
     ...
     * @return
     */
    public static <T extends BaseResults> FlowableTransformer<T, T> getApiTransformer() {
