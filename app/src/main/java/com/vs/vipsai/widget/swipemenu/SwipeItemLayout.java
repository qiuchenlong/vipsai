package com.vs.vipsai.widget.swipemenu;
...
public class SwipeItemLayout extends ViewGroup {
    ...
    void revise(){
        if(mScrollOffset>-mMaxScrollOffset/2 && mScrollOffset < 0) // ???????????????? if(mScrollOffset<-mMaxScrollOffset/2)
            open();
        else
            close();
    }
