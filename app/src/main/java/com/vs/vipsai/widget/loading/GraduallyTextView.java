...
public class GraduallyTextView extends EditText {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float w = mPaint.measureText(text.toString());
        float h = (float) (Math.ceil(fm.descent - fm.ascent) + 2);
        int width = getMeasuredWidth();
        int height = getMeasuredHeight();
        scaleX = getScaleX() + width / 2 - w / 2;
        startY = getScrollY() + height / 2;
        if (!isStop) {
            mPaint.setAlpha(255);
